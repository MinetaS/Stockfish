...
namespace {
  ...
  Value exsearch(Position &pos, Stack *ss, Move excludedMove,
...
//
// Attempt #3
// - Update some fields per move in exsearch. Thus Stack is not const in
//   exsearch, but only relevant fields should be updated not to mess with
//   the previous search state.

Value exsearch(Position &pos, Stack *ss, Move excludedMove,
               Value threshold, Depth depth, bool cutNode) {
    assert(is_ok(excludedMove));
    assert(VALUE_TB_LOSS_IN_MAX_PLY < threshold && threshold < VALUE_TB_WIN_IN_MAX_PLY);
    assert(0 < depth && depth < MAX_PLY);
    assert(0 <= ss->ply && ss->ply < MAX_PLY);
    assert(ss->inCheck || ss->staticEval != VALUE_NONE);

    Thread *const thread = pos.this_thread();

    // Step 1. Check if the position is repetitive.
    if (   pos.rule50_count() >= 3
        && threshold <= VALUE_DRAW
        && pos.has_game_cycle(ss->ply))
    {
        Value v = value_draw(thread);
        if (v >= threshold)
            return v;
    }
    ...
    // Skip ButterflyBoard update       [maybe] (~0.7 Elo)
    // Skip razoring                    [O]
    // Skip futility pruning            [maybe] (~0.3 Elo)
    ...
    const ButterflyHistory &mainHistory = thread->mainHistory;
    const CapturePieceToHistory &captureHistory = thread->captureHistory;
    ...
    while ((move = mp.next_move()) != MOVE_NONE) {
        ...
        Depth r = reduction(false, depth, moveCount, Value(1), thread->rootDelta);

        // Step 3. Pruning at shallow depth.
        if (pos.non_pawn_material(us) != VALUE_ZERO &&
            bestValue > VALUE_TB_LOSS_IN_MAX_PLY) {
            int lmrDepth = std::max(newDepth - r, 0);
            ...
            if (capture || check) {
                ...
                if (!check &&
                    lmrDepth < 7 &&
                    !ss->inCheck &&
                    ss->staticEval + 186 + 203 * lmrDepth + PieceValue[EG][pos.piece_on(to_sq(move))] +
                ...
                int history = (*contHistory[0])[movedPiece][to_sq(move)] +
                ...
                if (!ss->inCheck &&
                    lmrDepth < 13 &&
                    ss->staticEval + 103 + 136 * lmrDepth < threshold)
                    continue;

                lmrDepth = std::max(lmrDepth, 0);

                // SEE based pruning (~4 Elo)
                if (!pos.see_ge(move, Value(-25 * lmrDepth * lmrDepth - 16 * lmrDepth)))
                    continue;
            }
        }
        ...
        // Update stateful fields only.
        ss->currentMove = move;
        ss->continuationHistory = &thread->continuationHistory[ss->inCheck]
                                                              [capture]
                                                              [movedPiece]
                                                              [to_sq(move)];
        ss->statScore =  2 * mainHistory[us][from_to(move)]
                       + (*contHistory[0])[movedPiece][to_sq(move)]
                       + (*contHistory[1])[movedPiece][to_sq(move)]
                       + (*contHistory[3])[movedPiece][to_sq(move)]
                       - 4467;
        ...
        // Step 4. Full depth search.
        value = -search<NonPV>(pos, ss+1, -threshold, -threshold + 1, newDepth, !cutNode);
