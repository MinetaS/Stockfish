    ...
    // The depth condition is important for mate finding.
    {
        auto futility_margin = [&](Depth d) {
            ...
            return futilityMult * d                                //
                 - 2004 * improving * futilityMult / 1024          //
                 - 1326 * opponentWorsening * futilityMult / 4096  //
                 + (ss - 1)->statScore / 329                       //
                 + std::abs(correctionValue) / 149100;
        };
...
moves_loop:  // When in check, search starts here
    ...
    while ((move = mp.next_move()) != Move::none())
    {
        ...
        {
            Value singularBeta  = ttData.value - (56 + 85 * (ss->ttPv && !PvNode)) * depth / 60;
        ...
        if (ss->ttPv)
            r -= 2562 + PvNode * 1069 + (ttData.value > alpha) * 888
               + (ttData.depth >= depth) * (927 + cutNode * 1050);
