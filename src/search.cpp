  ...
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        && !(ttData.depth >= depth - 3 && ttData.value != VALUE_NONE && ttData.value < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != Move::none())
        {
            ...
            // Perform a preliminary shallow search to verify that the move holds
            value = -search<NonPV>(pos, ss + 1, -probCutBeta, -probCutBeta + 1, depth / 16, !cutNode);

            // If the previous search held, perform the regular search
