...
Value Search::Worker::search(
  Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        if (threads.stop.load(std::memory_order_relaxed) || pos.is_draw(ss->ply)
            ...
            return (ss->ply >= MAX_PLY && !ss->inCheck)
                   ? evaluate(networks[numaAccessToken], pos, refreshTable,
                              AccumulatorUpdateType::kCurrentOnly, thisThread->optimism[us])
    ...
    const AccumulatorUpdateType accUpdateType = PvNode || ss->moveCount > 1
                                                ? AccumulatorUpdateType::kCurrentAndOld
                                                : AccumulatorUpdateType::kCurrentOnly;
    ...
    else if (ss->ttHit)
    {
        ...
        if (unadjustedStaticEval == VALUE_NONE)
            unadjustedStaticEval = evaluate(networks[numaAccessToken], pos, refreshTable,
                                            accUpdateType, thisThread->optimism[us]);
        else if (PvNode)
            Eval::NNUE::hint_common_parent_position(pos, networks[numaAccessToken], refreshTable);
    ...
    {
        unadjustedStaticEval = evaluate(networks[numaAccessToken], pos, refreshTable, accUpdateType,
...
Value Search::Worker::qsearch(Position& pos, Stack* ss, Value alpha, Value beta) {
    ...
    const AccumulatorUpdateType accUpdateType =
      PvNode ? AccumulatorUpdateType::kCurrentAndOld : AccumulatorUpdateType::kCurrentOnly;
